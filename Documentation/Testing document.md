...
This class contains one test. It checks that all the numbers that were on the list before it was shuffled are still present after it has been shuffled.

## Data structures
### StackTest
This class tests the stack's methods (push(), pop(), peek(), size(), isEmpty()) by adding and removing values from it.
...
### UnionFindTest
The union-find data structure is tested by checking if different nodes are connected when they should be and if the structure finds the correct root node.


# Performance tests
The performance tests are part of the program and can be run by the user. When the program is started there is a button that says "Performance tests" in the menu. When this button is clicked the user is taken to a view where they can start the tests. The performance tests are run by the class called "PerformanceComparator". It makes the different algorithms perform their task multiple times and displays the results after the test is done. There are three parts.
...
# Test data
There is also a class called "Datagathering". This class runs the same tests as the PerformanceComparator class, but it runs them 100 times and creates text files containing the test data. The mazes' proportions are 3000 * 3000. Below are some examples of the results.
